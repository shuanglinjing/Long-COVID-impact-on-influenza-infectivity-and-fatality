...
for j=1:length(K) 
    [t,y] = ode15s(@Model_Flu_ode,time,y0,[],beta1(j,:)',beta2(j,:)');
    S=y(:,1);E1=y(:,2);E2=y(:,3);I1=y(:,4);I2=y(:,5);R=y(:,6);
    N=(S+E1+E2+I1+I2+R);
    for i=1:length(time1)
        ERN1=effective_reproduction_number(beta1(j,i),beta2(j,i),S(i),N(i));
...
for j=1:length(K) 
    ...
    N=(S+E1+E2+I1+I2+R);
    for i=1:length(time1)
        ERN2=effective_reproduction_number(beta1(j,i),beta2(j,i),S(i),N(i));
...
for j=1:length(K) 
    ...
    for i=1:length(time2)
        ERN3=effective_reproduction_number(beta1(j,i),beta2(j,i),S(i),N(i));
...
for j=1:length(K) 
    ...
    N=(S+E1+E2+I1+I2+R);
    for i=1:length(time1)
        ERN5=effective_reproduction_number(beta1(j,i),beta2(j,i),S(i),N(i));
...
for j=1:length(K) 
    ...
    N=(S+E1+E2+I1+I2+R);
    for i=1:length(time1)
        ERN6=effective_reproduction_number(beta1(j,i),beta2(j,i),S(i),N(i));
...
[ERN_fy1,ERN_fx1,ERN_Mean1]= modelCI(time1,ERN_Flu1');
[ERN_fy2,ERN_fx2,ERN_Mean2]= modelCI(time1,ERN_Flu2');
[ERN_fy3,ERN_fx3,ERN_Mean3]= modelCI(time2,ERN_Flu3');
[ERN_fy5,ERN_fx5,ERN_Mean5]= modelCI(time1,ERN_Flu5');
[ERN_fy6,ERN_fx6,ERN_Mean6]= modelCI(time1,ERN_Flu6');

save('Re_MS.mat','ERN_fy1','ERN_fx1','ERN_Mean1',...
